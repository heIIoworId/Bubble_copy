...
void main() {
    ...
    vec4 xbump = texture2D(uTextureNormalUnit, wPosition.yz * scale);
    vec4 ybump = texture2D(uTextureNormalUnit, wPosition.xz * scale);
    vec4 zbump = texture2D(uTextureNormalUnit, wPosition.xy * scale);

    vec4 tex = xaxis * blending.x + yaxis * blending.y + zaxis * blending.z;
    vec4 bump = xbump * blending.x + ybump * blending.y + zbump * blending.z;

    bumpNormal = normalize(bump.rgb * 2.0 - 1.0);

    // change normal to normal + bumpnormal
